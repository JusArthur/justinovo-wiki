...
export default function RecommendationsPage({ lang = "EN" }) {
    ...
    const filteredData = recommendationsData.filter(item => {
      ...
      const matchesCategory = activeCategory === "全部" || item.tags.includes(activeCategory) || item.category === activeCategory;
              ...
              >
                ...
                <div className="mb-4 flex items-center gap-4">
                  <div className="group relative shrink-0">
                    <img src={item.image} alt={item.title} className="h-16 w-16 rounded-xl object-cover shadow-sm" />
                  </div>
                  <div className="flex-1 overflow-hidden">
                    ...
                    <a href={item.url} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-[#35bfab] dark:text-gray-400 dark:hover:text-[#39ff14] mt-1 block max-w-full truncate text-xs hover:underline">
                      {item.url}
                    </a>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-0.5">
                  {[...Array(5)].map((_, i) => (
                    <svg key={i} width="16" height="16" viewBox="0 0 24 24" className={i < item.stars ? "fill-yellow-400" : "fill-gray-300 dark:fill-gray-600"}>
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {item.tags.map(tag => (
                    <span key={tag} className="bg-[#35bfab]/10 text-[#35bfab] dark:bg-[#39ff14]/10 dark:text-[#39ff14] rounded-full px-2.5 py-0.5 text-xs font-medium border border-[#35bfab]/20 dark:border-[#39ff14]/20">
                      {tag}
                    </span>
                  ))}
                </div>
